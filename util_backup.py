...
def korrigiere_entlade_lade_bilanz(strategie, soc_differenz, flexband, fahrplan_zeitraum, min_soc, basis_soc, mitte, capacity):
    ...
    for i in lade_punkte:
        ...
        neuer_soc = korrigierte_strategie[i]["soc"] - (korrektur_pro_punkt / 4)
...
def implementiere_strategien(strategien_json, fahrplan_json, user_inputs_json):
    ...
    try:
        ...
        for strategie in strategien:
            ...
            # Vorab prüfen, ob Strategie SoC-Limits verletzen würde
            # Simuliere SoC-Verlauf für diesen Zeitraum
            test_soc = flexband[start_idx]["soc"] if start_idx < len(flexband) else 0.3 * capacity
            soc_valid = True
            
            for i in range(start_idx, end_idx + 1):
                if i > start_idx:
                    # Apply previous combined action
                    prev_idx = i - 1
                    combined_action = neuer_fahrplan[prev_idx]["value"]
                    # Find corresponding strategy detail
                    for d in strategie["strategie_details"]:
                        if d["index"] == prev_idx:
                            combined_action += d["aktion"]
                            break
                    test_soc += combined_action / 4
                
                # Check if SoC would violate limits
                if test_soc < 0.05 * capacity - 1 or test_soc > 0.95 * capacity + 1:  # Small tolerance
                    soc_valid = False
                    break
            
            if not soc_valid:
                continue  # Skip this strategy as it would violate SoC limits
